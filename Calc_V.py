...
#Calculate Vf or Vm
def calc_V(flux_bottom,case,parameters_medium,_zi_,z_bot_i,zw_,zsurf,R_IN,R_OUT,R_DRILL,deltaE,nbr_points_interpolation_K,AveragingK_):
    ...
    if (case==1234 or case==2134 or case==1243 or case==2314 or case==2143):
         ...
         K_INT=t1[0]
         dK_INT=t1[1]         
         vint=(2*R_DRILL*(zw_-_zi_)*K_INT)/((R_IN*R_IN+0.4*R_DRILL*R_DRILL-0.4*R_OUT*R_OUT)*deltaE)
         u=(zw_-_zi_)
         uprime=1.0
         v=K_INT
         vprime=dK_INT
         dvint=(2*R_DRILL/((R_IN*R_IN+0.4*R_DRILL*R_DRILL-0.4*R_OUT*R_OUT)*deltaE))*((u*vprime)+(v*uprime))
         if (flux_bottom==True):
         ...
         integrale=t2[0]
         dintegrale=t2[1]
         v_piezo_vide=(2*R_DRILL*Ks_loc*integrale)/((R_IN*R_IN+0.4*R_DRILL*R_DRILL-0.4*R_OUT*R_OUT)*deltaE)
         dv_piezo_vide=(2*R_DRILL*Ks_loc*dintegrale)/((R_IN*R_IN+0.4*R_DRILL*R_DRILL-0.4*R_OUT*R_OUT)*deltaE)
         ...
         K_INT=t3[0]
         dK_INT=t3[1] 
         v_piezo_plein=2*R_DRILL*(zw_-_zi_)*K_INT/((R_IN*R_IN+0.4*R_DRILL*R_DRILL-0.4*R_OUT*R_OUT)*deltaE)
         u=(zw_-_zi_)
         uprime=1.0
         v=K_INT
         vprime=dK_INT
         dv_piezo_plein=(2*R_DRILL/((R_IN*R_IN+0.4*R_DRILL*R_DRILL-0.4*R_OUT*R_OUT)*deltaE))*((u*vprime)+(v*uprime))
         if (flux_bottom==True):
