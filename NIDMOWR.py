...
#Read params_well
ztube,zsurf_,zbot_f,zbot_m,rin_,rout_,rdrill_,deltaE_=np.loadtxt(fname=Test_case+"/Inputs/params_well.txt", dtype=float, usecols=1)
...
while (index_time<nbr_dates_interp-1):
         ...
         while (convergence==False):
             ...
             t4=calc_V(R_IN=rin_,R_OUT=rout_,R_DRILL=rdrill_,flux_bottom=False,case=case_m,parameters_medium=param_mat,_zi_=zm,z_bot_i=zbot_m,
             ...
             t5=calc_V(R_IN=rin_,R_OUT=rout_,R_DRILL=rdrill_,flux_bottom=False,case=case_f,parameters_medium=param_fract,_zi_=zf,z_bot_i=zbot_f,
                   zsurf=zsurf_,deltaE=deltaE_,nbr_points_interpolation_K=nbr_pts_rect_int_K ,
                   zw_=zw_it[index_iterations],AveragingK_=AveragingK)
             Vfract=t5[0]
             dVfract=t5[1]
             ...
             if (abs(F[index_iterations])<epsilon):
                 ...
                 area=3.14159265359*rin_*rin_+0.4*3.14159265359*(rdrill_*rdrill_-rout_*rout_)
